...
class Settings(BaseSettings):
    ...
    NK_CONCURRENCY: int = 4
    # Повторы при 429 (лимит НК) и 5xx — с экспоненциальным бэкоффом / Retry-After.
    NK_MAX_RETRIES: int = 4
    # Лимит метода v3/product — 100 запросов в «серии» с окном 5 минут (докум. ЦРПТ).
    # Держим ~1 запрос / 3.1с (≈100 за 5 мин), чтобы не упираться в 429. Троттлинг
    # ГЛОБАЛЬНЫЙ для процесса (общий ключ apikey), поэтому реальная конкурентность = 1.
    NK_MIN_INTERVAL_MS: int = 3100
    # Потолок ожидания Retry-After при 429 (сек) — окно серии максимум 5 минут.
    NK_MAX_COOLDOWN_S: float = 330.0
    # Бюджет синхронного дозаполнения имён в приёмке (сек): НК тормозит троттлингом,
    # поэтому в hot-path приёмки ограничиваем — что не успели, доберётся из кэша позже.
    NK_ACCEPTANCE_BUDGET_S: float = 8.0
